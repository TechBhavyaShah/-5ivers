...
router.post(
  ...
  async (request, response) => {
    try {
      ...
      validator.isPostFoodItemFieldsValid(Object.keys(requestPostData).length);

      const restaurantId = validator.isRestaurantIdValid(
        xss(decodedAccessToken.restaurant?.id)
      );

      const name = validator.isFoodItemNameValid(xss(requestPostData.name));
      ...
      const type = validator.isFoodItemTypeValid(xss(requestPostData.type));
...
router.put(
  ...
  async (request, response) => {
    try {
      ...
      validator.isPutFoodItemStockValid(Object.keys(requestPostData).length);
